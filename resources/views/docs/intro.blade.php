...
Routes marked with the <x-docs.scope-badge anchor scope="OAuth" /> label require a valid OAuth2 token for access.
...
@php
    ...
    $endpoint = new \Knuckles\Camel\Output\OutputEndpointData([
...
[Client Credentials Grant](#client-credentials-grant) tokens do not have a Resource Owner (i.e. is a guest user), unless they have been granted the <x-docs.scope-badge scope="delegate" /> scope. The Resource Owner of tokens with the <x-docs.scope-badge scope="delegate" /> scope is the owner of the OAuth Application that was granted the token.
...
Client Credentials Grant tokens may be allowed to act on behalf of the owner of the OAuth client (delegation) by requesting the <x-docs.scope-badge scope="delegate" /> scope, in addition to other scopes supporting delegation.
When using delegation, scopes that support delegation cannot be used together with scopes that do not support delegation.
Delegation is only available to [Chat Bot]({{ wiki_url('Bot_account', null, false) }})s.

The following scopes currently support delegation:

- <x-docs.scope-badge scope="chat.write" />
...
<x-docs.scope-badge anchor :scope="$scope" /> | {{ $description }}
@endforeach
...
Routes marked with <x-docs.scope-badge anchor scope="lazer" /> are intended for use by the [osu!lazer](https://github.com/ppy/osu) client and not currently available for use with Authorization Code or Client Credentials grants.

Using the <x-docs.scope-badge scope="chat.write" /> scope requires either
- a [Chat Bot]({{ wiki_url('Bot_account', null, false) }}) account to send messages on behalf of other users.
